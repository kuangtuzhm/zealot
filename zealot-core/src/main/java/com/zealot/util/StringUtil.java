...
public class StringUtil extends StringUtils{
	...
	public static Integer[] strArrayToIntArray(String[] array)
	{
		...
		Integer[] itypes=new Integer[array.length];
		
		int index=0;
    	for(int i=0;i<itypes.length;i++)
    	{
    		if(RegexUtil.isNumber(array[i]))
    		{
    			itypes[index++]=Integer.parseInt(array[i]);
    		}
    	}
    	
    	return Arrays.copyOf(itypes,index);
	}
     ...
     * @return
     */
    public static ArrayList<String> divideString2(String value, String separator){
    ...
    public static Integer[] splitInteger(String value) {
        return splitInteger(value,",");
    }
    
    public static Integer[] splitInteger(String value, String separator) {
        String[] intss = StringUtil.splitString(value, separator);
        if(intss!=null && intss.length>0){
            Integer[] temInts = new Integer[intss.length];
            for(int i = 0; i<intss.length; i++){
                if(!StringUtil.isEmpty(intss[i])){
                    temInts[i] = Integer.parseInt(intss[i].trim());
                }
            }
            return temInts;
        }
